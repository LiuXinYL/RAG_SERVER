    ...
    for child in element:
        ...
        if child_type != next_level_type:
            ...
            for grandchild in child:
                grandchild_copy = ET.Element(grandchild.tag)
                grandchild_copy.attrib = grandchild.attrib.copy()
                child_copy.append(grandchild_copy)
            # 确保子节点有闭合标签
            if not list(child_copy):
                child_copy.text = ""
            new_element.append(child_copy)

    # 确保当前节点有闭合标签
    if not list(new_element):
        new_element.text = ""

    xml_str = '<?xml version="1.0" encoding="UTF-8" standalone="yes" ?>\n'
    xml_str += ET.tostring(new_element, encoding='unicode', method='xml').replace('""', '"')
